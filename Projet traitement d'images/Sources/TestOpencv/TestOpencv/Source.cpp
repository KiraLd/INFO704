...
int main(int argc, char** argv)
{
	...
	if (argc < 3)
	{
		...
		cout << " Par défaut maxStrokeLength = 16, erroApproximation = 100, curvature = 1.0, output_file = output.jpg"<<endl;
		return -1;
	}
	...
	int c;
	Paint p;
	while ((c = getopt(argc, argv, "T:C:M:I:O:")) != -1)
	{
		switch (c)
		{
		case 'T':
		...
		case 'C':
			...
			break;
		case 'M':
			p.setStrokeLength(4, atoi(optarg));
			break;
		case 'I':
