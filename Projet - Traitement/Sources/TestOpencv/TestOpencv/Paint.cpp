...
void Paint::setMinLength(int m)
{
	min = m;
}

void Paint::setMaxLength(int m)
{
	max = m;
}

void Paint::setRatio(float r)
{
	ratio = r;
}

void Paint::setBrushMax(int m)
{
	R = m;
}

void Paint::setNbBrushes(int N)
{
	n = N;
}
...
void Paint::paintLayer(int R)
{
	...
	if (R == this->R)
	{
		for (int i = 0; i < canvas.size[0]; i++)
		{
			...
			{
				d[i*canvas.size[1] + j] = (float)(2 * T + 1);
			}
		}
	}
	else
